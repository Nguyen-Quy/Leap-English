...
# from Shop_Books.settings import EMAIL_HOST_USER
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = 'Cảm ơn quý khách <b>' + order.last_name + \
                ' ' + order.first_name + '</b> đã đặt hàng tại LEAP English,'
            recepient = str(email_address)

            html_content = '''
            <p style="margin:4px 0;font-family:Arial, Helvetica, sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;">LEAP English rất vui thông báo đơn hàng ''' + \
                str(order.id) + \
            ...
            <p style="margin:0;font-family:Arial, Helvetica, sans-serif;font-size:12px;color:#444;line-height:18px;font-weight:normal;">Trường hợp quý khách có những băn khoăn về đơn hàng, có thể liên hệ với chúng tôi theo số <b>0963 645 899</b></p>'''
            ...
            return render(request, "orders/order/created.html", {'cart': cart, 'order': order, 'username': username})
    else:
        form = OrderCreateForm(instance=order)
    return render(request, "orders/order/create.html", {'cart': cart, 'form': form, 'username': username})
